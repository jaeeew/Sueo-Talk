...
for idx, npz_file in enumerate(os.listdir(NPZ_DIR)):
    if not npz_file.endswith(".npz"):
        continue
    npz_path = os.path.join(NPZ_DIR, npz_file)
    data = np.load(npz_path)
    for video_key in data.files:
        video_data = data[video_key]
        X_list.append(video_data)
        y_list.append(idx)

X = tf.keras.preprocessing.sequence.pad_sequences(X_list, padding="post", dtype="float32")
y = np.array(y_list)
classes = [f"{i+1}번 문장" for i in range(len(np.unique(y)))]
...
def augment_to_balance(X, y, min_samples_per_class=100):
    X_augmented = list(X)
    y_augmented = list(y)
...
print(f"훈련 데이터: {len(X_train)} → 증강 후 {len(X_train_aug)}")
...
callbacks = [
    ...
    ModelCheckpoint("best_lstm_model.keras", monitor='val_accuracy', save_best_only=True, verbose=1)  # .keras로 저장!
]
