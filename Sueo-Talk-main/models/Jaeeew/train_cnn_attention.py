...
from tensorflow.keras.layers import Input, Conv1D, Dense, Dropout, GlobalAveragePooling1D, LayerNormalization
...
class Attention(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        score = tf.nn.tanh(self.W(inputs))  # (batch, time, 1)
        weights = tf.nn.softmax(score, axis=1)  # (batch, time, 1)
        context_vector = weights * inputs  # (batch, time, features)
        return tf.reduce_sum(context_vector, axis=1)  # (batch, features)

# --- ✅ Temporal Convolution + Attention 모델 구성 ---
...
# 모델 저장 (옵션)
# model.save("sign_model_temporal_cnn_attention.h5")
